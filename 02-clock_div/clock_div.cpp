...
#include "obj_dir/Vclock_div.h"

constexpr unsigned MAX_STEPS = 1 << 20;

void tick(unsigned tickCount, Vclock_div *tb, VerilatedVcdC *trace)
{
    tb->eval();
...
int main(int argc, char **argv)
{
    ...
    auto tb = std::make_unique<Vclock_div>();

    tb->trace(trace.get(), 99);
    trace->open("clock_divtrace.vcd");
